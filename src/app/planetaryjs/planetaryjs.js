...
// TODO ______________________________________________________________________________________________________
    planetaryjs.plugins.pingMarkers = (selector, config) => {

        var pings = [];
        var zoomAngle = config.angle || 5;
        config = config || {};

        // Copied partially from Planetary.js's pings plugin
        var addPing = function (lng, lat, options) {
            options = options || {};
            options.color = options.color || config.color || 'white';
            options.angle = options.angle || config.angle || 5;

            if (config.randomTtl && config.randomTtl.enabled) {
                options.ttl = Math.random() * ((config.randomTtl.max || 2500) - (config.randomTtl.min || 1500)) + (config.randomTtl.min || 1500);
            } else {
                options.ttl = options.ttl || config.ttl || 2000;
            }
        ...
        var drawPings = function (planet, now) {
            var temp;
            for (var i = 0; i < pings.length; i++) {
                var alive = now - pings[i].time;
                if (alive > pings[i].options.ttl) {
                    temp = pings[i];
                    pings.splice(i, 1);

                    temp.time = now;
                    if (config.zoomAngle && config.zoomAngle.enabled) {
                        temp.angle = zoomAngle;
                        temp.options.angle = zoomAngle;
                    }

                    pings.push(temp);
                    planet.plugins.pings.add(temp.lng, temp.lat, temp.options);
                }
            }
        };

        var getPoint = function (lng, lat, errorRadius) {
            var point = null;
            for (var i = 0; i < pings.length; i++) {
                if ((pings[i].lng >= lng - errorRadius && pings[i].lng <= lng + errorRadius) &&
                    (pings[i].lat >= lat - errorRadius && pings[i].lat <= lat + errorRadius)) {
                    point = pings[i];
                    break;
                }
            }
        ...
        var zoom = function (event) {
            if (config.zoomAngle && config.zoomAngle.enabled) {
                if (!config.zoomAngle.onZoom || config.zoomAngle.onZoom == 'default') {
                    var currentScale = event.scale;
                    var scale = {min: event.target.scaleExtent()[0], max: event.target.scaleExtent()[1]};
                    var range = {min: config.zoomAngle.min || 1, max: config.zoomAngle.max || 3};

                    var newAngle = ((currentScale - scale.min) / (scale.max - scale.min)) * (range.max - range.min) + range.min;
                    zoomAngle = newAngle;
                } else {
                    zoomAngle = config.zoomAngle.onZoom();
                }
            }
        };

        return function (planet) {
            planet.plugins.pingMarkers = {
                add: addPing,
                pings: pings,
                shouldRotate: true,
                zoom: zoom
            ...
            planet.onDraw(function () {
                var now = new Date();
                drawPings(planet, now);
            });

            d3.select(selector).on("mousedown.log", function () {
                var clickpoint = planet.projection.invert(d3.mouse(this));

                var pt = getPoint(clickpoint[0], clickpoint[1], config.errorMargin || 1);
                if (pt != null) {
                    planet.plugins.autorotate.pause();
                    planet.plugins.pingMarkers.shouldRotate = false;
                    $(planet.plugins.pingMarkers).trigger('click', {lng: pt.lng, lat: pt.lat});

                    // TODO: See fixme above
                    // TODO: Fire event on jQuery or on planet.plugins.pingMarkers and figure that out
                } else {
                    planet.plugins.pingMarkers.shouldRotate = true;
                }

                // TODO: Create looping pings plugin and use it
                // TODO: Stop globe from rotating upon click
                // TODO: Resume rotation only if it's not an existing ping
                // TODO: If not an existing ping, rotate it so that that point is at the center and bring up certain gui features
            });
        };
    };
